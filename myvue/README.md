...
接着遍历老节点,通过map查找，卸载不存在的，留下可以复用的，并且更新array,将对应新节点下标位置的值改成老节点下标+1，（+1是因为可能第一位就找到了下标就会是0），根据当前节点在新节点中的位置和上次复用节点在新节点中的位置判断，这次大于上次的，说明是升序的，更新复用节点位置，不需要移动。否则就是需要移动的。就需要移动，后面倒序遍历新节点完成后续操作之前就会求一个最长递增子序列。以尽可能的减少dom操作。如果不存在可移动的，就不会生成最长递增子序列。