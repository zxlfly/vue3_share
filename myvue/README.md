...
接着遍历老节点,通过map查找，卸载不存在的，留下可以复用的，并且更新array。根据当前节点在新节点中的位置和之前复用节点对应在新节点中的最靠后的一个位置判断，这次大于之前的，（老节点按顺序遍历的是升序，新节点复用也是升序）说明是升序的，不需要移动，更新最靠后的复用节点位置。否则就是需要移动的。然后patch可复用节点。
遍历处理完之后，如果需要移动就会通过array求最长递增子序列，这个稳定序列返回的是对应的数组索引，也就是新子节点的下标。接着倒叙遍历新子节点，会通过前面生成的数组判断如果对应的值是0就是新增，当前遍历节点索引不在稳定序列中就需要移动，再就直接过，前面已经patch过了。
异步更新策略